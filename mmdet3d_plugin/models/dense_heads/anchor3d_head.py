...
@HEADS.register_module()
class LogitKDAnchor3DHead(Anchor3DHead):
    def __init__(self, 
                num_classes, 
                in_channels, 
                train_cfg, test_cfg, 
                ...
                loss_bbox=dict(type='SmoothL1Loss', beta=1 / 9, loss_weight=2), 
                loss_dir=dict(type='CrossEntropyLoss', loss_weight=0.2), 
                init_cfg=None):
        super(LogitKDAnchor3DHead, self).__init__(num_classes, in_channels, train_cfg, test_cfg, feat_channels, use_direction_classifier, anchor_generator, assigner_per_size, assign_per_class, diff_rad_by_sin, dir_offset, dir_limit_offset, bbox_coder, loss_cls, loss_bbox, loss_dir, init_cfg)
    ...
    def logit_distillation(self, stu_scores, tea_scores, label_weight, avg_factor):
        """
            Args:
                stu_score: tensor, shape=[N, C]
                tea_score: tensor, shape=[N, c]
                label_weight: tensor. shape=[N]
                avg_factor: tensor
            Return:
                loss_kd: tensor
        """
    ...
    def loss_single(self, cls_score, bbox_pred, dir_cls_preds, tea_cls_score, labels,
                    label_weights, bbox_targets, bbox_weights, dir_targets,
                    dir_weights, num_total_samples):
        """Calculate loss of Single-level results.
        ...
        Args:
            ...
            dir_cls_preds (torch.Tensor): Predictions of direction class
                in single-level.
            tea_cls_score (torch.Tensor): Class score in single-level from teacher.
            labels (torch.Tensor): Labels of class.
            label_weights (torch.Tensor): Weights of class loss.
            bbox_targets (torch.Tensor): Targets of bbox predictions.
            bbox_weights (torch.Tensor): Weights of bbox loss.
        ...
        # Logit KD
        tea_cls_score = tea_cls_score.permute(0, 2, 3, 1).reshape(-1, self.num_classes)
        ...
        return loss_cls, loss_bbox, loss_dir, loss_kd
    ...
    def loss(self,
        ...
        Args:
            ...
            dir_cls_preds (list[torch.Tensor]): Multi-level direction
                class predictions.
            tea_cls_scores (list[torch.Tensor]): Multi-level class scores from teacher.
            gt_bboxes (list[:obj:`BaseInstance3DBoxes`]): Gt bboxes
                of each sample.
            gt_labels (list[torch.Tensor]): Gt labels of each sample.
            input_metas (list[dict]): Contain pcd and img's meta info.
        ...
        # num_total_samples = None
        losses_cls, losses_bbox, losses_dir, losses_kd = multi_apply(
            self.loss_single,
            cls_scores,
            bbox_preds,
            dir_cls_preds,
            ...
            num_total_samples=num_total_samples)
        return dict(
            loss_cls=losses_cls, loss_bbox=losses_bbox, loss_dir=losses_dir, loss_logitkd=losses_kd)
