...
model = dict(
    ...
    distillation=dict(
        kd_type="voxel",
        loss_balance=[0.25, 0.25],
        local_cfg=dict(
            SG_layers=3,
            num_samplings = [4000, 2000, 1000],  # [3000, 1500, 750]
            num_neighbors = [96, 48, 24],  # [64, 32, 16]
            tea_in_channels = 64,
            tea_out_channels = 64,
            stu_in_channels = 16,
            stu_out_channels = 16,
    ...
    voxel_encoder=dict(
        ...
        point_cloud_range=[0, -39.68, -3, 69.12, 39.68, 1]),
    middle_encoder=dict(
        type='PointPillarsScatter', in_channels=16, output_shape=[496, 432]),
    backbone=dict(
        type='SECOND',
        in_channels=16,  # 64
        layer_nums=[3, 5, 5],
    ...
    neck=dict(
        type='SECONDFPN',
        in_channels=[16, 32, 64],  # [64, 128, 256]
        upsample_strides=[1, 2, 4],
        out_channels=[32, 32, 32]),  # [128, 128, 128]
    bbox_head=dict(
        type='Anchor3DHead',
        num_classes=3,
        in_channels=384//4,  # 384
        feat_channels=384//4,  # 384
